...
export default function PatternAnalysis({ initialMessages }: PatternAnalysisProps) { 
  ...
  useEffect(() => { 
    const performAnalysis = async () => { // Made this an async function
      // Try to get messages from prop or local storage 
      const messages = initialMessages || JSON.parse(localStorage.getItem('chatMessages') || '[]');
      ...
      try { 
        // Await the result of analyzeContent 
        const detectedPatterns = await analyzeContent(messages); 
